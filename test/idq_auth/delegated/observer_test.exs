defmodule IdqAuth.DelegatedObserverTest do
  ...
  describe "expected return with callback arity one" do
  ...
  describe "expected return with callback arity two" do
  ...
  describe "start/4" do
    ...
    test "returns a tuple with :ok atom, and pid" do
      {:ok, pid} = start("TITLE", "MESSAGE", 10, &callback_arity_one/1, %{})
      assert is_pid(pid)
    end
  end
  ...
  defp start_server_arity_two(_context) do
    {:ok, pid} = start("TITLE", "MESSAGE", 10, &callback_arity_two/2, %{"context" => "CONTEXT"})
    {:ok, %{pid: pid}}
  end
